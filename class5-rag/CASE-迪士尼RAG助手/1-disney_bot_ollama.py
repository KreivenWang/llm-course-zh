...
1. 安装所有必需的 Python 库:
   pip install openai "faiss-cpu" python-docx PyMuPDF Pillow paddleocr transformers torch requests sentence-transformers

2. 安装并启动 Ollama:
   - 从 https://ollama.ai/ 下载安装 Ollama
   - 运行 `ollama pull qwen3-coder:30b` 下载模型
...
TEXT_EMBEDDING_DIM = 768  # Using a smaller dimension for local model
...
def image_to_text(image_path):
    ...
    try:
        ...
        ocr_text = ""
        # PaddleOCR result format: [Page1, Page2, ...] where each page is [WordBox1, WordBox2, ...]
        # Each WordBox is [bbox, (text, confidence)]
        for page_result in result:
            if page_result:  # Check if page result is not None/empty
                for detection in page_result:
                    if detection:
                        text = detection[1][0]  # Extract text from (text, confidence) tuple
                        ocr_text += text + " "
...
"""
    ...
    try:
        completion = client.chat.completions.create(
            model="qwen3-coder:30b", # 使用本地 qwen3-coder:30b 模型
            messages=[
                {"role": "system", "content": "你是一个迪士尼客服助手。"},
                {"role": "user", "content": prompt}
            ],
            temperature=0.1  # 设置较低的温度以获得更一致的回答
        )
